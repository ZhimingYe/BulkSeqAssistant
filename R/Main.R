...
doKEGG<-function(GS,PVal=0.01,QVal=0.05,Organism="hsa",OrgDB=org.Hs.eg.db){
  ...
  #library(org.Hs.eg.db)
  GeneList<-GS%>%unique()%>%bitr(fromType = "SYMBOL",toType = "ENTREZID",OrgDb = OrgDB)
  EnrichKEGG<-enrichKEGG(GeneList$ENTREZID,pvalueCutoff = PVal,qvalueCutoff = QVal,organism=Organism)%>%setReadable(OrgDb=OrgDB,keyType = "ENTREZID")
  return(EnrichKEGG)
}
...
#' @return A ggplot2 object
#' @export
#' @author Zhiming Ye, Drawing codes cited from ggbiplot.R <- Copyright 2011 Vincent Q. Vu.
#'
#' @examples
...
#' @param OrgDB The annotation DB, default as org.Hs.eg.db
...
#'
#' @examples
MapSymbol <- function(TargetGene, ColNum=1, OrgDB=org.Hs.eg.db,Type="to") {
...
#' @title Convert counts to TPM
#'
#' @param Counts Expression matrix
#' @param Length length vector of gene
#' @param gene names of genes
#'
#' @return FPKM matrix
#' @export
#' @author Zhiming Ye
#'
#' @examples
countToFPKM <- function(Counts,Length,gene)
{
  if(length(Length)!=length(gene)){stop("vector length unequal to vector gene\n")}
  Counts->Exprset
  countToFpkm2 <- function(counts, effLen){
    N <- sum(counts)
